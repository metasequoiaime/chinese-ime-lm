...
**与其自己训，不如直接用开源小模型？** 在同一套候选池上量过：Qwen3-0.6B 与 Qwen3.5-0.8B 打平本项目 4.5 MB 的模型，Qwen3.5-2B 也没有超过 25.5 MB 的那个，而体积差 140 到 380 倍、单次决策慢一个数量级。同一次测量还发现整句评测集里有 12 条「他/她」不可判的用例，因此本项目两个模型之间的排名暂时不能当结论——两半都记在 [`docs/measurements.md`](docs/measurements.md#与开源通用模型比较)。
