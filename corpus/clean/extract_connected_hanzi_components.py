"""
替换所有文本中的非中文字符为空格，并且将每个汉字的连通块写入新的一行
"""
        ...
        for line in infile:
            ...
            for word in words:
                if word:  # 如果是非空词，才写入
                    ...
                    if TOTAL_WRITTEN_LINES % 10000 == 0:
                        print(f"{TOTAL_WRITTEN_LINES // 10000} 万行")


def iter_input_files(data_dir):
    """递归收集 data/wiki_zh 下的所有文件，并附加 data 目录下以 train.json 结尾的文件"""
...
def main():
    ...
    output_file = os.path.join(base_dir, "data/output/all_cleaned.txt")
